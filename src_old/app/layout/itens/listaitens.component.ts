...
@Component({
    selector: 'app-listaitens',
    templateUrl: './listaitens.component.html',
    styleUrls: ['./listaitens.component.scss'],
...
export class ListaItensComponent implements OnInit {
    ...
    carregaLista(): void {
        ...
        this.itemService.listItems(this.idRequest).pipe(takeUntil(this.destroy$)).subscribe((data: any[]) => {
            this.isLoadPanelVisible = false;
            this.itens = data;
        },
        error => {
    ...
    novoItem() {
        this.router.navigate(['item'], { queryParams: { 'idRequest' : this.idRequest } });
    }
